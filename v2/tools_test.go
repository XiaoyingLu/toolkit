...
var jsonTests = []struct {
	...
	{name: "incorrect type", json: `{"foo":2}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "two json files", json: `{"foo":"bar"}{"alpha":"beta"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "empty body", json: ``, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "syntax error in json", json: `{"foo":}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		// call the function and decode the json
...
func TestTools_PushJSONToRemote(t *testing.T) {
	...
	if err != nil {
		t.Error("failed to execute PushJSONToRemote():", err)
	}
}
